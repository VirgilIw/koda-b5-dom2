...
const fetchDataPoke = async (urlPoke, urlType) => {
  try {
    ...
    // Render DOM
    detailPokemons.forEach((detailPokemon) => {
      const sprites = detailPokemon.sprites;
      console.log(sprites);

      const img = document.createElement("img");
      img.src = sprites.front_shiny;
      // gambar
      img.alt = detailPokemon.name;

      const name = document.createElement("p");
      name.textContent = detailPokemon.name;

      const detailtype = document.createElement("p");
      //
      detailtype.textContent = detailPokemon.types
        .map((item) => {
